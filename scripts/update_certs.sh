...
om --env "${HOME_DIR}/om_${ENV_NAME}.env"  \
 configure-director --config ${TEMPLATE_DIR}/director_certs.yml --vars-file ${TEMPLATE_DIR}/director_certs_vars.yml
...
cat << EOF > ${TEMPLATE_DIR}/pas_cert_vars.yml
...
om --env "${HOME_DIR}/om_${ENV_NAME}.env"  \
  configure-product \
  -c ${TEMPLATE_DIR}/pas_cert.yml -l ${TEMPLATE_DIR}/pas_cert_vars.yml